...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, in_dim=64, dim=96):
        ...
        self.conv_down = nn.Sequential(
            nn.Conv2d(in_chans, in_dim, 1, 1, 1, bias=False),
            nn.BatchNorm2d(in_dim, eps=1e-4),
            nn.ReLU(),
            nn.Conv2d(in_dim, dim, 1, 1, 1, bias=False),
...
class GuidanceMambaVisionMixer(nn.Module):
    def __init__(
        self,
        d_model,
        d_state=16,
        ...
        self.in_proj_guidance = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)    
        self.x_proj_guidance = nn.Linear(
            self.d_inner//2, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        )
        self.dt_proj_guidance = nn.Linear(self.dt_rank, self.d_inner//2, bias=True, **factory_kwargs)
        ...
        self.A_log_guidance = nn.Parameter(A_log)
        self.A_log_guidance._no_weight_decay = True
        self.D_guidance = nn.Parameter(torch.ones(self.d_inner//2, device=device))
        self.D_guidance._no_weight_decay = True
        self.out_proj_guidance = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        self.conv1d_x_guidance = nn.Conv1d(
            in_channels=self.d_inner//2,
            out_channels=self.d_inner//2,
            bias=conv_bias//2,
            kernel_size=d_conv,
        ...
        self.conv1d_z_guidance = nn.Conv1d(
            in_channels=self.d_inner//2,
            out_channels=self.d_inner//2,
            bias=conv_bias//2,
            kernel_size=d_conv,
    ...
    def forward(self, hidden_states):
        ...
        """
        _, seqlen, _ = hidden_states.shape
        hidden_states_half1, hidden_states_half2 = hidden_states.chunk(2, dim=-1)
        
        xz = self.in_proj(hidden_states_half1)
        xz = rearrange(xz, "b l d -> b d l")
        x, z = xz.chunk(2, dim=1)
        A = -torch.exp(self.A_log.float())
        x = F.silu(F.conv1d(input=x, weight=self.conv1d_x.weight, bias=self.conv1d_x.bias, padding='same', groups=self.d_inner//2))
        ...
        hidden_states_xz = self.in_proj_guidance(hidden_states_half2)
        hidden_states_xz = rearrange(hidden_states_xz, "b l d -> b d l")
        hidden_states_x, hidden_states_z = hidden_states_xz.chunk(2, dim=1)
        A_guidance = -torch.exp(self.A_log_guidance.float())
        hidden_states_x = F.silu(F.conv1d(input=hidden_states_x, weight=self.conv1d_x_guidance.weight, bias=self.conv1d_x_guidance.bias, padding='same', groups=self.d_inner//2))
        hidden_states_z = F.silu(F.conv1d(input=hidden_states_z, weight=self.conv1d_z_guidance.weight, bias=self.conv1d_z_guidance.bias, padding='same', groups=self.d_inner//2))
        x_dbl_guidance = self.x_proj_guidance(rearrange(x, "b d l -> (b l) d"))
        dt_guidance, B_guidance, C_guidance = torch.split(x_dbl_guidance, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt_guidance = rearrange(self.dt_proj_guidance(dt_guidance), "(b l) d -> b d l", l=seqlen)
        B_guidance = rearrange(B_guidance, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
        C_guidance = rearrange(C_guidance, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
        y_guidance = selective_scan_fn(hidden_states_x, 
                              dt_guidance, 
                              A_guidance, 
                              B_guidance, 
                              C_guidance, 
                              self.D.float(), 
                              z=None, 
                              delta_bias=self.dt_proj_guidance.bias.float(), 
                              delta_softplus=True, 
                              return_last_state=None)
        y = torch.cat([y, hidden_states_z], dim=1)
        y = rearrange(y, "b d l -> b l d")
        out = self.out_proj(y)
        
        y_guidance = torch.cat([y_guidance, z], dim=1)
        y_guidance = rearrange(y_guidance, "b d l -> b l d")
        out_guidance= self.out_proj_guidance(y_guidance)
        
        out = torch.cat([out, out_guidance], dim=-1)
        return out
...
class Block(nn.Module):
    def __init__(self, 
        ...
        #     self.mixer = GuidanceMambaVisionMixer(d_model=dim, 
        #                                   d_state=8,  
        #                                   d_conv=3,    
        #                                   expand=1
        #  )
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        self.downchannel = nn.Conv2d(in_dim, in_dim//2,  kernel_size=1, stride=1, bias=False)
        self.guidance_norm = nn.LayerNorm(in_dim) 
    ...
    def forward(self, x, appearance_guidance):
        B, C, T, H, W = x.shape
        x = rearrange(x, 'B C T H W -> (B T) C H W', B=B, C=C,T=T, H=H, W=W)
        if appearance_guidance is not None:
            if len(appearance_guidance.size())==5:
                appearance_guidance = rearrange(appearance_guidance, 'B C T H W -> (B T) C H W', B=B, C=C,T=T, H=H, W=W)
            else:
        ...
        x = rearrange(x, '(B T) C H W ->B C T H W ', B=B, C=C,T=T, H=H, W=W)
        # x = self.head(x)
        return x
...
def mamba_vision_L2_512_21k(pretrained=False, **kwargs):
    model_path = kwargs.pop("model_224path", "/tmp/mamba_vision_L2_512_21k.pth.tar")
    depths = kwargs.pop("depths", [3, 3, 12, 5])
    num_heads = kwargs.pop("num_heads", [4, 8, 16, 32])
...
if __name__ == "__main__":
    model = mamba_vision_Ts(pretrained=False).cuda()

...
class ConMB_SS2D(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x_rgb = x_rgb.permute(0, 2, 3, 1).contiguous() # (b, d, h, w)
        x_e = x_e.permute(0, 2, 3, 1).contiguous() # (b, d, h, w)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
            self.mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, channels_first=False)
        self.in_txt = nn.Linear(128, hidden_dim)
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        B, C, T, H, W = x_rgb.shape
        x_rgb = rearrange(x_rgb, 'B C T H W -> (B T) C H W', B=B, C=C,T=T, H=H, W=W)
        if x_e is not None:
            if len(x_e.size())==5:
                x_e = rearrange(x_e, 'B C T H W -> (B T) C H W', B=B, C=C,T=T, H=H, W=W)
                print("x_e shape before in_txt", x_e.shape)
                x_e = self.in_txt(x_e.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
                print("x_e shape after in_txt", x_e.shape)
            else:
                x_e = repeat(x_e, 'B C H W -> (B T) C H W', T=T)
        
        print("x_e shape", x_e.shape)
        x = self.drop_path(self.op(x_rgb, x_e))
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        x = x.permute(0, 3, 1, 2).contiguous() # (B T) H W C

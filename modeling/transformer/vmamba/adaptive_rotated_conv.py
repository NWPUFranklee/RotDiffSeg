...
class AdaptiveRotatedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 kernel_number=1, rounting_func=None, rounting_func1=None, rotate_func=batch_rotate_multiweight):
        super().__init__()
        ...
        self.rounting_func1 = rounting_func1
    ...
    def forward(self, x, x_e):
        ...
        alphas_e, angles_e = self.rounting_func1(x_e)
        # rotate weight
        # # [Cout, Cin, k, k] --> [bs * Cout, Cin, k, k]
        print("alpha shape" ,alphas.shape)
        print("angle shape" ,angles.shape)

        print("alphae shape" ,alphas_e.shape)
        print("anglee shape" , angles_e.shape)
        alphas_cat = torch.cat((alphas, alphas_e), dim=0)
        angles_cat = torch.cat((angles, angles_e), dim=0)
        print("alphas_cat shape", alphas_cat.shape)
        print("angles_cat shape", angles_cat.shape)

        rotated_weight_x = self.rotate_func(self.weight + self.weight1, alphas + alphas_e, angles)
        rotated_weight_x_e = self.rotate_func(self.weight + self.weight1, alphas_e + alphas, angles_e)

        # print("rotated_weight_x shape", x.shape)
        # print("rotated_weight_x_e shape", x_e.shape)
        # reshape images
        bs, Cin, h, w = x.shape
        x = x.reshape(1, bs * Cin, h, w)  # [1, bs * Cin, h, w]
        x_e = x_e.reshape(1, bs * Cin, h, w)

        # adaptive conv over images using group conv
        out = F.conv2d(input=x, weight=rotated_weight_x, bias=None, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=(self.groups * bs))
        out_e = F.conv2d(input=x_e, weight=rotated_weight_x_e, bias=None, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=(self.groups * bs))
        # reshape back
        print("rotated_weight_x shape", out.shape)
        print("rotated_weight_x_e shape", out_e.shape)
        out = out.reshape(bs, self.out_channels, *out.shape[2:])
        out_e = out_e.reshape(bs, self.out_channels, *out_e.shape[2:])
        return out, out_e

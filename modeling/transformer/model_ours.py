...
from .mamba.mambafusion_text import mamba_vision_Tt
from .mamba.mambafusion import mamba_vision_Ts
...
#         super().__init__()
#         self.nheads = nheads
#         self.q = nn.Linear(guidance_dim, hidden_dim)
...
from .models.WPAttention import MultiheadAttention
from .models.WPAttention import MSCW
import math
class CrossAttentionLayer(nn.Module):
    def __init__(self, hidden_dim, guidance_dim, nheads=8, attention_type='linear'):
        super().__init__()
        self.nheads = nheads
        self.q = nn.Linear(guidance_dim + hidden_dim, hidden_dim)
        self.k = nn.Linear(hidden_dim, hidden_dim)
        self.v = nn.Linear(hidden_dim, hidden_dim)
        self.pool = wavelet.WavePool(hidden_dim)
        self.pool1 = wavelet.WavePool(guidance_dim)
        self.mscw1 = MSCW(d_model=hidden_dim)
        self.mscw2 = MSCW(d_model=guidance_dim)
        self.up1 = nn.ConvTranspose2d(hidden_dim, hidden_dim, kernel_size=2, stride=2)
        self.up2 = nn.ConvTranspose2d(hidden_dim, hidden_dim, kernel_size=2, stride=2)
        self.up3 = nn.ConvTranspose2d(guidance_dim, guidance_dim, kernel_size=2, stride=2)
        self.up4 = nn.ConvTranspose2d(guidance_dim, guidance_dim, kernel_size=2, stride=2)

        self.cov1 = nn.ConvTranspose2d(hidden_dim * 3, hidden_dim, kernel_size=2, stride=2)
        self.cov2 = nn.ConvTranspose2d(guidance_dim * 3, guidance_dim, kernel_size=2, stride=2)
        if attention_type == 'linear':
            self.attention = LinearAttention()
        elif attention_type == 'full':
            self.attention = FullAttention()
    ...
    def forward(self, x, guidance):
        ...
        feat = x.transpose(1, 2).view(b, c, hw, hw)
        LL, HL, LH, HH = self.pool(feat)
        high_fre = HL + LH + HH
        # high_fre = HH
        # high_fre = self.cov1(torch.cat([HL, LH, HH], dim=1))
        low_fre = LL
        high_fre = self.up1(high_fre)
        low_fre = self.up2(low_fre)
        ...
        b, n1, c = guidance.size()
        hw = int(math.sqrt(n1))
        feat_guidance = guidance.transpose(1, 2).view(b, c, hw, hw)
        LL_guidance, HL_guidance, LH_guidance, HH_guidance = self.pool1(feat_guidance)
        high_fre_guidance = HL_guidance + LH_guidance + HH_guidance
        # high_fre_guidance = self.cov2(torch.cat([HL_guidance, LH_guidance, HH_guidance], dim=1))
        # high_fre_guidance = HH_guidance
        low_fre_guidance = LL_guidance
        high_fre_guidance = self.up3(high_fre_guidance)
        low_fre_guidance = self.up4(low_fre_guidance)
        high_fre_guidance = high_fre_guidance.flatten(2).transpose(1, 2)
        low_fre_guidance = low_fre_guidance.flatten(2).transpose(1, 2)
        wei_guidance = self.mscw2(high_fre_guidance + low_fre_guidance)
        fre_guidance = wei_guidance*high_fre_guidance + low_fre_guidance

        q = self.q(torch.cat([fre_guidance, fre], dim=-1))
        k = self.k(x)
        v = self.v(x)
...
class ClassTransformerLayer(nn.Module):
    def __init__(self, hidden_dim=64, guidance_dim=64, nheads=8, attention_type='linear', pooling_size=(4, 4), pad_len=256) -> None:
        ...
        # self.attention = mamba_vision_Tt(dim=256)
        hidden_dim = 128
        cfg = dict(
        ...
        self.attention = ConcatMambaFusionBlock(**cfg)
    ...
    def forward(self, x, guidance):
        ...
        *_, H_pool, W_pool = x_pool.size()
        ...
        guidance = rearrange(guidance, '(B H W) T C -> B C T H W', H=H_pool, W=W_pool)            
        x_pool = self.attention(x_pool, guidance)
        x_pool = rearrange(x_pool, 'B C T H W -> (B H W) T C')
        # x_pool = x_pool + self.attention(self.norm1(x_pool), guidance) # Attention
        # x_pool = x_pool + appearance_guidanceself.MLP(self.norm2(x_pool)) # MLP        
...
#         self.pool = nn.AvgPool2d(pooling_size) if pooling_size is not None else nn.Identity()
#         self.attention = AttentionLayer(hidden_dim, guidance_dim, nheads=nheads, attention_type=attention_type)
#         # self.attention = CrossAttentionLayer(hidden_dim, guidance_dim, nheads=nheads, attention_type=attention_type)
#         self.MLP = nn.Sequential(
#             nn.Linear(hidden_dim, hidden_dim * 4),
#             nn.ReLU(),
#             nn.Linear(hidden_dim * 4, hidden_dim)
...
#         """
#         B, C, T, H, W = x.size()
#         print("ClassTransformerLayer input x shape:", x.shape)
#         x_pool = self.pool_features(x)
#         print("ClassTransformerLayer pooled x shape:", x_pool.shape)
#         *_, H_pool, W_pool = x_pool.size()
...
#                     guidance = torch.cat([guidance, padding_guidance], dim=1)
#             guidance = repeat(guidance, 'B T C -> (B H W) T C', H=H_pool, W=W_pool)
#         print("ClassTransformerLayer x_pool shape before attention:", x_pool.shape)
#         print ("ClassTransformerLayer guidance shape before attention:", guidance.shape if guidance is not None else None)
#         x_pool = x_pool + self.attention(self.norm1(x_pool), guidance) # Attention
#         print("ClassTransformerLayer x_pool shape after attention:", x_pool.shape)
...
from .vmamba.vmamba import ConcatMambaFusionBlock
class AggregatorLayer(nn.Module):
    def __init__(self, hidden_dim=64, text_guidance_dim=512, appearance_guidance=512, nheads=4, input_resolution=(20, 20), pooling_size=(5, 5), window_size=(10, 10), attention_type='linear', pad_len=256) -> None:
        ...
        # self.swin_block = mamba_vision_Ts(dim=256)
        hidden_dim = 128
        cfg = dict(
            hidden_dim=hidden_dim,
            drop_path=0.0,
        ...
        self.swin_block = ConcatMambaFusionBlock(**cfg)
    ...
    def forward(self, x, appearance_guidance, text_guidance):
        ...
        Arguments:
            x: B C T H W
        """
        print("x shaped: ", x.shape)
        print("app shape: ", appearance_guidance.shape)
...
from .vmamba.routing_function import RountingFunction
from .vmamba.adaptive_rotated_conv import AdaptiveRotatedConv2d    
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, guidance_channels):
        ...
        self.channel_unified = nn.ConvTranspose2d(guidance_channels, in_channels - guidance_channels, kernel_size=1, stride=1)
        self.conv = DoubleConv((in_channels - guidance_channels)*2, out_channels)
        self.cconv = nn.Conv2d(in_channels - guidance_channels, in_channels - guidance_channels, kernel_size=1, stride=1, padding=0)
        self.sconv = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)

        self.routing_function1 = RountingFunction(in_channels=in_channels - guidance_channels, kernel_number=1)
        self.routing_function2 = RountingFunction(in_channels=in_channels - guidance_channels, kernel_number=1)
        self.conv2_4 = AdaptiveRotatedConv2d(in_channels=in_channels - guidance_channels, out_channels=in_channels - guidance_channels,
                                    kernel_size=3, padding=1, rounting_func=self.routing_function1, rounting_func1=self.routing_function2, bias=False, kernel_number=1)

    def forward(self, x, guidance=None):
        guidance_x = guidance
        x = self.up(x)
        guidance = self.channel_unified(guidance) if guidance is not None else None
        T = x.size(0) // guidance.size(0)
        guidance = repeat(guidance, "B C H W -> (B T) C H W", T=T)

        x, guidance = self.conv2_4(x, guidance)
        # guidance = guidance.view(guidance_x.size(0),T,x.size(1),guidance_x.size(2),guidance_x.size(3))
        print("Up layer x shape after upsample:", x.shape)
        print("Up layer guidance shape:", guidance.shape if guidance is not None else None)
        if guidance is not None:
            # T = x.size(0) // guidance.size(0)
            attention = x.view(guidance_x.size(0)*T,x.size(1),guidance_x.size(2),guidance_x.size(3))
            channel_attention = attention.mean(dim=[2,3],keepdim=True)
            spatial_attention = attention.mean(dim=1,keepdim=True)

            channel_attention = self.cconv(channel_attention)
            spatial_attention = self.sconv(spatial_attention)
            print("channel_attention shape:", channel_attention.shape)
            print("spatial_attention shape:", spatial_attention.shape)
            guidance = F.sigmoid(channel_attention)*guidance + F.sigmoid(spatial_attention)*guidance + guidance
            
            # guidance = repeat(guidance, "B C H W -> (B T) C H W", T=T)
            # guidance_x = repeat(guidance_x, "B C H W -> (B T) C H W", T=T)
...
#         # self.conv = DoubleConv(guidance_channels, out_channels)
#         self.cconv = nn.Conv2d(in_channels - guidance_channels, guidance_channels, kernel_size=1, stride=1, padding=0)
#         self.sconv = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)
#         self.attn = AttentionLayer(in_channels - guidance_channels, guidance_channels, nheads=8, attention_type='linear')
#         self.attn1 = CrossAttentionLayer(in_channels - guidance_channels, guidance_channels, nheads=8, attention_type='linear')
#         self.attn2 = CrossAttentionLayer(guidance_channels, in_channels - guidance_channels, nheads=8, attention_type='linear')
#         self.norm1 = nn.LayerNorm(in_channels - guidance_channels)
#         self.norm2 = nn.LayerNorm(guidance_channels)
#         self.MLP = nn.Sequential(
#             nn.Linear(in_channels - guidance_channels, (in_channels - guidance_channels) * 4),
#             nn.ReLU(),
#             nn.Linear((in_channels - guidance_channels) * 4, in_channels - guidance_channels)
#         )
#         self.MLP1 = nn.Sequential(
#             nn.Linear(guidance_channels, guidance_channels * 4),
#             nn.ReLU(),
#             nn.Linear(guidance_channels * 4, guidance_channels)
#         )
...
#         if guidance is not None:
#             T = x.size(0) // guidance.size(0)
#             # attention = x.view(guidance.size(0),T,x.size(1),guidance.size(2),guidance.size(3)).mean(dim=1)
#             # channel_attention = attention.mean(dim=[2,3],keepdim=True)
#             # spatial_attention = attention.mean(dim=1,keepdim=True)
...
#             # guidance = channel_attention*guidance + spatial_attention*guidance + guidance
#             print("guidance shape:", guidance.shape)
#             guidance = repeat(guidance, "B C H W -> (B T) C H W", T=T)
#             print("after repeat guidance shape:", guidance.shape)
#             # x = torch.cat([x, guidance], dim=1)
#             x = rearrange(x, 'B C H W -> B (H W) C')
#             guidance = rearrange(guidance, 'B C H W -> B (H W) C')
#             mlp_x = self.attn1(self.norm1(x), self.norm2(guidance)) + x
#             # mlp_guidance = self.attn2(self.norm2(guidance), self.norm1(x)) + guidance
#             x = self.MLP(self.norm1(mlp_x)) + mlp_x
#             # y = self.MLP1(self.norm2(mlp_guidance)) + mlp_guidance
#             print("after attention x shape:", x.shape)
#             x = rearrange(x, 'B (H W) C -> B C H W', H=H, W=W)
#             # y = rearrange(y, 'B (H W) C -> B C H W', H=H, W=W)
#             # x = torch.cat([x, y], dim=1)
#         return self.conv(x)

# from .mamba_decoder.mam_attn_local import SimHymbaSwinDec
# class Up(nn.Module):
#     """Upscaling then double conv"""

#     def __init__(self, in_channels, out_channels, guidance_channels, seq_len):
#         super().__init__()
...
#         self.attn = SimHymbaSwinDec(embed_dim_att=in_channels , embed_dim_att1=guidance_channels, num_attention_heads=8, window_size=4, shift_size=2,
#                  mamba_states=16, seq_len=seq_len, hidden_size=in_channels, hidden_size1=guidance_channels, value_dim=in_channels + guidance_channels, ith_layer=0).cuda()

#         print("in_channels:", in_channels, "guidance_channels:", guidance_channels, "out_channels:", out_channels)
#         self.norm1 = nn.LayerNorm(in_channels - guidance_channels)
#         self.norm2 = nn.LayerNorm(guidance_channels)
#         self.MLP = nn.Sequential(
#             nn.Linear(in_channels - guidance_channels, (in_channels - guidance_channels) * 4),
...
#             x = rearrange(x, 'B C H W -> B (H W) C')
#             guidance = rearrange(guidance, 'B C H W -> B (H W) C')
#             print("before attention x shape:", x.shape)
#             print ("before attention guidance shape:", guidance.shape)
#             print("self.in_channels:", self.in_channels, "self.guidance_channels:", self.guidance_channels)
#             x = self.attn(x, guidance)
#             print("after attention x shape:", x.shape)
#             x = rearrange(x, 'B (H W) C -> B C H W', H=H, W=W)
...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        # self.decoder4 = Up(decoder_dims[2], decoder_dims[3], decoder_guidance_proj_dims[3])
    ...
    def correlation(self, img_feats, text_feats):
        ...
        print("image feature shape:", img_feats.shape)
        text_feats = F.normalize(text_feats, dim=-1) # B T P C
        print("text feature shape:", text_feats.shape)
        corr = torch.einsum('bnchw, btpc -> bnpthw', img_feats, text_feats)
        print("correlation shape before rearrange:", corr.shape)
        corr = rearrange(corr, 'B N P T H W -> B (N P) T H W')
        print("correlation shape after rearrange:", corr.shape)
        return corr

    def corr_embed(self, x):
        B = x.shape[0]
        corr_embed = rearrange(x, 'B P T H W -> (B T) P H W')
        print("correlation embedding shape before conv:", corr_embed.shape)
        corr_embed = self.conv1(corr_embed)
        print("correlation embedding shape after conv:", corr_embed.shape)
        corr_embed = rearrange(corr_embed, '(B T) C H W -> B C T H W', B=B)
        print("correlation embedding shape after rearrange:", corr_embed.shape)
        return corr_embed
    ...
    def conv_decoder(self, x, guidance):
        ...
        corr_embed = rearrange(x, 'B C T H W -> (B T) C H W')
        print("Decoder input shape after rearrange:", corr_embed.shape)
        print("Guidance shape before projection:", guidance[0].shape)
        corr_embed = self.decoder1(corr_embed, guidance[0])
        print("Decoder shape after first upsample:", corr_embed.shape)
        print ("Guidance shape before second projection:", guidance[1].shape)
        corr_embed = self.decoder2(corr_embed, guidance[1])
        print("Decoder shape after second upsampl   e:", corr_embed.shape)
        # corr_embed = self.decoder3(corr_embed, guidance[2])
        # corr_embed = self.decoder4(corr_embed, guidance[3])
        corr_embed = self.head(corr_embed)
        print("Decoder output shape before rearrange:", corr_embed.shape)
        corr_embed = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)
        print("Decoder output shape after rearrange:", corr_embed.shape)
        return corr_embed
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        """
        classes = None
        print("Input image feature shape:", img_feats[0].shape)
        corr = self.correlation(img_feats, text_feats)
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len: #B (N P) T H W 
            avg = corr.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0]  #B  T (N P) H W 
            ...
            # text_feats = th_text
            # corr1 = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text)

        # corr2 = self.correlation(img_feats[1], text_feats)
        # if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
        #     avg = corr2.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] 
        #     classes = avg.topk(self.pad_len, dim=-1, sorted=False)[1]
        #     th_text = F.normalize(text_feats, dim=-1)
        #     th_text = torch.gather(th_text, dim=1, index=classes[..., None, None].expand(-1, -1, th_text.size(-2), th_text.size(-1)))
        #     orig_clases = text_feats.size(1)
        #     img_feats = F.normalize(img_feats, dim=1) # B C H W
        #     text_feats = th_text
        #     corr2 = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text)

        # corr3 = self.correlation(img_feats[2], text_feats)
        # if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
        #     avg = corr3.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] 
        #     classes = avg.topk(self.pad_len, dim=-1, sorted=False)[1]
        #     th_text = F.normalize(text_feats, dim=-1)
        #     th_text = torch.gather(th_text, dim=1, index=classes[..., None, None].expand(-1, -1, th_text.size(-2), th_text.size(-1)))
        #     orig_clases = text_feats.size(1)
        #     img_feats = F.normalize(img_feats, dim=1) # B C H W
        #     text_feats = th_text
        #     corr3 = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text)

        # corr4 = self.correlation(img_feats[3], text_feats)
        # if self.pad_len > 0 and text_fea32ts.size(1) > self.pad_len:
        #     avg = corr4.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] 
        #     classes = avg.topk(self.pad_len, dim=-1, sorted=False)[1]
        #     th_text = F.normalize(text_feats, dim=-1)
        #     th_text = torch.gather(th_text, dim=1, index=classes[..., None, None].expand(-1, -1, th_text.size(-2), th_text.size(-1)))
        #     orig_clases = text_feats.size(1)
        #     img_feats = F.normalize(img_feats, dim=1) # B C H W
        #     text_feats = th_text
        #     corr4 = torch.einsum('bchw, btpc -> bpthw', img_feats, th_text)
        ...
        if self.guidance_projection is not None:
            print("111")
            print("appearance guidance[0] shape:", appearance_guidance[0].shape)
            projected_guidance = self.guidance_projection(appearance_guidance[0])
            print("projected guidance shape:", projected_guidance.shape)
        if self.decoder_guidance_projection is not None:
            ...
            print("projected decoder guidance shapes:", [g.shape for g in projected_decoder_guidance])
        if self.text_guidance_projection is not None:
            print("text features shape before projection:", text_feats.shape)
            text_feats = text_feats.mean(dim=-2)
            text_feats = text_feats / text_feats.norm(dim=-1, keepdim=True)
            projected_text_guidance = self.text_guidance_projection(text_feats)
            print("projected text guidance shape:", projected_text_guidance.shape)
